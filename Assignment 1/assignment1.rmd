...
knn_more = function(X, X0, y, K){
  ...
  classifs = c()
  for (g in y) {
    if (g %in% classifs == FALSE) {
      classifs = c(classifs, g, 0)
    }
  }

  # loop through classifications to count
  cnts = matrix(classifs, nrow=length(classifs)/2, byrow=TRUE)
  for (g in NN_classif) {
    cnts[g,2] = cnts[g,2] + 1
  }
...
int knn_more(NumericMatrix X, NumericVector X0, NumericVector y, int K) {
    ...
    int j = 0;
    for (i = 0; i < nrows; i++) {
      if (contains(classifs, y[i]) == false) {
        j = j+1;
      }
    }
    NumericVector classifs(j);
    for (i = 0; i < j; i++) {
      classifs[i] = i+1;
    }
